...
def generate_report_with_gemini_search():
    ...
    prompt = f"""
    今日は {today} です。
    最新のディープフェイク（Deepfake）に関するニュースや動向（過去24時間〜48時間以内）を調査し、日本語で網羅的なレポートを作成してください。

    {additional_instr}

    【検索と収集の指示 - グローバル対応】
    - **マルチリンガル検索**: 日本語だけでなく、必ず英語等の多言語で検索（例: "deepfake news breaking", "AI fraud 2026", "new deepfake regulations world"）を行い、世界中の最新情報を収集してください。
    - **ソースの構成**: レポートの50%以上は、海外の主要メディア、テックブログ、政府発表などの国際的なソースに基づく内容にしてください（内容は全て日本語に翻訳すること）。
    - **重複と既知情報の排除**: 過去数週間にわたって報じられている「トレンド」や「一般的なツール紹介」ではなく、**「ここ1〜2日の間に起きた具体的なニュース・事件・発表」**を最優先してください。

    【レポートの構成 - 厳格守付】
    1. **主要ニュースのまとめ (10件〜15件程度)**
       - 各項目には以下の内容を必ず含めてください：
         - 番号付きの見出し：記事のタイトル（※必ず日本語に翻訳）
         - 内容の要約：具体的かつ客観的に3〜4文程度で記述
         - 出典：必ず `出典: [メディア名・サイト名](URL)` の Markdown 形式で記述（※URLは絶対に省略せず、メディア名も日本語化して記述）
    
    2. **その他の注目見出し (可能な限り多数)**
       - 世界各地の短報、技術リリース等を、タイトルと出典リンクの形式でリストアップしてください。
       - 形式：`* [記事タイトル（要日本語翻訳）](URL) - メディア名`

    【最重要：禁止事項と制約】
    - **完全日本語化**: ソースが何語であっても、要約やメディア名に至るまで全て流暢な日本語で出力してください。
    - **リンクの網羅**: 出典には必ず `[テキスト](URL)` 形式のURLを含めてください。
    - **冒頭・結びの不要**: 「承知いたしました」等の挨拶は一切不要です。
    - **鮮度の徹底**: 3日以上前の古いニュース（既に広く知られている一般論など）は含めないでください。
    """
